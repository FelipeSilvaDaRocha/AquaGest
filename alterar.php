<?php
    ...
    $alerta_membro_alterado = isset($_GET['warning']) ? $_GET['warning'] : 0;
...
<body>
    ...
    <main>
        <h2>Alterar Informações de Membro</h2>
        <div id="cadastroNovoMembro">
            <form action="processa_cadastro.php" method="get">
                ...
                <input type="text" id="apelido" name="apelido" value="<?php echo $apelido ?>">
    
                <label for="data_nascimento">Data de Nascimento:</label>
                <input type="date" id="data_nascimento" name="data_nascimento" value="<?php echo $data_nascimento ?>" required>

                <label for="celular">celular:</label>
                <input type="tel" id="celular" name="celular" value="<?php echo $celular ?>" required>
        
                <label for="rua">Nome da Rua:</label>
                <select id="selecionarRuaNovoMembro" name="rua" style="padding: 8px; margin: 8px 0; border-color: #ccc; width: 99.5%; border-radius: 4px" required>
                    <?php
                        echo '<option selected>'.$nome_rua.'</option>';
                ...
                <input type="number" id="numero" name="numero" value="<?php echo $numero_casa ?>" required>
                ...
                <?php
                    if($status == 'Ativo'){
                        echo "<select id='status' name='status' style='padding: 8px; margin: 8px 0; border-color: #ccc; width: 99.5%; border-radius: 4px' required>";
                            echo '<option selected>'.$status.'</option>';
                            echo '<option >Ativo</option>';
                            echo '<option >Inativo</option>';
                    }else{
                        echo "<select id='status' name='status' style='padding: 8px; margin: 8px 0; border-color: #F44336; width: 99.5%; border-radius: 4px' required>";
                            echo '<option selected>'.$status.'</option>';
                            echo '<option >Ativo</option>';
                            echo '<option >Inativo</option>';
                    }
                ...
                <input type="hidden" name="id" value="<?php echo $id_membro ?>">
    
                <button type="update">Alterar</button>
            </form>
        </div>
